...
const Signup = () => {
  ...
  const handleContinue = async () => {
    ...
    try {
        setLoading(true);
        console.log(`Searching for student with matric_no: ${inputValue}`);

        // Simulate API call with local data
        const student = students.find((student) => student.matric_no === inputValue);
        
        if (student) {
            setErrorMsg(""); // Clear error message if found
            router.push(`/student/getstarted/createpassword/${student._id}`);
        } else {
            setErrorMsg("Matriculation number not found.");
        }
    } catch (err) {
        console.error("Error caught in handleContinue:", err);
